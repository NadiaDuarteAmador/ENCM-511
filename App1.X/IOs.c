...
{  
    ...
    else if (pb_state == 1) // PB1 pressed
    {
        ...
        Disp2Dec(seconds);
        Disp2String("s");
        XmitUART2('\r', 1); // message to print display once, next line
       
        if(PORTAbits.RA2 == 1) // PB1 no longer being pressed
        {
            pb_state = 0; // Back to initial state, Led Off and Idle. Waits for timer to be started
    ...
    else if (pb_state == 2) // PB2 pressed
    {
        ...
        {
            pb_state = 0; // Back to initial state, Led Off and Idle. Waits for for timer to be started
        ...
        //Check the state of each button when the interrupt is triggered to filter debounce effect in buttons
         uint16_t seconds_counter = 0;
            ...
            while ((PORTAbits.RA2 == 1) && (PORTBbits.RB4 == 1) && (PORTAbits.RA4 == 0)) // While PB3 pressed 
            {
                ...
                XmitUART2('\r', 1);  // next line for start and stop timer message                        
                XmitUART2(' ', 10);  // spaces for next line                    
                XmitUART2('\r', 1);                             
