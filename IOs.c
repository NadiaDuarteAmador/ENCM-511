...
{  
    ...
    else if (x == 2) 
    {
        ...
        if (seconds > 59) // no more than 59 seconds
        {
    ...
    else if (x == 4) // PB3 pressed for more than 3s
    {
        Disp2String("00m : 00s"); // Reset timer 
        seconds = 0; // Reset seconds
        minutes = 0; // Reset minutes
        start_pause = 0; // Initialize timer again
        x = 0; // Back to initial state
    ...
    else if (x == 3) // PB3 pressed for less than 3s
    {
        if (start_pause == 1)
        {
            ...
            {
                seconds = 59; // Begin second countdown in next minute
                minutes = minutes - 1; // Move on with next minute
            }
            
            if ((minutes == 0) && (seconds == 0)) // When countdown is complete 
            {
            ...
            if (start_pause == 0) // PB3 pressed, Paused
            {
                x = 0; // Back to initial state
    ...
    if(IFS1bits.CNIF ==1){  
        ...
        //Check the state of each button when the interrupt is triggered to filter debounce effect in buttons
         uint16_t seconds_counter = 0;
            ...
            {
                x = 1; // Variable x gets assigned an arbitrarily value to recognize a specific state in IOCheck
            }
